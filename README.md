...
Open the app at **http://localhost:5173/hex-stats/** (Vite `base` is `/hex-stats/` for GitHub Pages).

Build: `npm run build`, preview: `npm run preview` (preview at **http://localhost:4173/hex-stats/**).

After re-running the crawler at the repo root, run `npm run sync-stats` inside `web/` so the UI loads fresh data.

### GitHub Pages (`https://<user>.github.io/hex-stats/`)
...
2. Copy **everything** under `dist/` into the `hex-stats/` folder in the `disper.github.io` repo: `index.html`, `assets/`, `data.js`, and `armiesStats.json` (optional; `data.js` already embeds stats when generated from `armiesStats.json`). The `npm run deploy-to-github` script automates this via `rsync --delete`.
3. Commit and push in `disper.github.io`.

> The app previously lived at `https://disper.github.io/stats/`. That path now serves a meta-refresh redirect to `/hex-stats/`.
